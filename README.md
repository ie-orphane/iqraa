...
### `The table public.connections does not exist`

Migrations were not applied.

```bash
bun run supabase:status   # confirm DB is up
bun run db:migrate:deploy
```

### `Bind for 0.0.0.0:54322 failed: port is already allocated`

Another local Supabase project is using default ports. Iqraa uses `54422` — ensure `.env.local` points at `54422` and `supabase/config.toml` matches.

### `Can't reach database server at 127.0.0.1:54422`

```bash
bun run supabase:start
```

Confirm Docker Desktop / Docker Engine is running (especially on WSL).

### Discord avatar / `hostname resolved to private IP`

WSL DNS64 quirk. The app sets `images.dangerouslyAllowLocalIP` and uses `unoptimized` for Discord avatars.

### Hydration warning with `cz-shortcut-listen`

Browser extension (e.g. ColorZilla), not app code. `<body>` uses `suppressHydrationWarning`. Test in a private window to confirm.

### Prisma + Turbopack module errors

Client is generated into `node_modules/@prisma/client`. Packages are externalized in `next.config.ts`. If things break after schema changes:
